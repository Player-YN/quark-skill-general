...
官方 CLI **没有**文件删除命令。当用户要求删除网盘文件或文件夹时，agent 必须将「删除」理解为：把目标 **move** 到名为恰好「回收站」的文件夹。禁止编造或调用不存在的 `delete` 命令。

这不是官方回收站：文件仍占用网盘容量。移动完成后，必须告知用户可在夸克 App 中删除「回收站」文件夹，以真正释放空间。

#### 每次删除前必须探测

每次执行删除语义时，都必须重新探测，禁止沿用会话中缓存的回收站 FID：

```bash
node scripts/quark-drive.cjs search --keyword "回收站" --search-type dir --stdout-only
```

探测搜索仅作为中间步骤获取文件夹 FID，**必须**传入 `--stdout-only`，结果不向用户展示。

按探测结果分支：

| 探测结果 | agent 行为 |
| --- | --- |
| 恰好 1 个名为「回收站」的文件夹 | 使用该文件夹 FID，将目标 `move` 进去 |
| 0 个 | 在网盘**根目录**创建：`create-folder --dir-path "回收站" --parent-fid "0"`，再将目标 `move` 进去。用户已明确要求放在顶层 |
| 多个 | 停止并询问用户选用哪一个，禁止猜测 |

创建「回收站」时必须传入 `--parent-fid "0"`（用户明确选择顶层）；不得省略该参数，以免落到平台默认目录。

#### 移动约束

- `move` 目标 FID 最多 100 个，超出时分批。
- **禁止**将「回收站」文件夹自身 move 进自己。
- **禁止**在「回收站」内再创建名为「回收站」的嵌套文件夹。

#### 完成后告知用户

移动成功后，告知用户：文件已移入「回收站」，仍占用容量；如需彻底删除并释放空间，请在夸克 App 中删除「回收站」文件夹。
