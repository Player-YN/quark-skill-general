...
16. **删除语义走回收站**：用户要求删除网盘文件或文件夹时，禁止调用不存在的 delete 命令；每次必须先 `search --keyword "回收站" --search-type dir --stdout-only` 探测，再将目标 `move` 进名为恰好「回收站」的文件夹。无则在根目录创建（`--parent-fid "0"`），多个则询问用户、禁止猜测。详见 [文件操作](#文件操作)
...
官方 CLI 没有文件删除命令。用户说「删除」时，将目标 `move` 到名为恰好「回收站」的文件夹：每次探测（`--stdout-only`），恰好一个则移入，没有则在根目录 `create-folder --dir-path "回收站" --parent-fid "0"` 后移入，多个则询问用户、禁止猜测。禁止把「回收站」移入自身或在其中再创建「回收站」。此操作不释放容量，完成后告知用户可在夸克 App 中删除「回收站」文件夹。完整步骤见 [references/file-ops.md](references/file-ops.md)「删除语义 / 回收站」。
